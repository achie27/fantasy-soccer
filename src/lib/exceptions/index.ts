export class BaseContextualError extends Error {
  ...
  constructor(message: string, code: string, httpResponseCode: number, notes?: Record<string, any>) {
    super(message);
...
export class InternalServerError extends BaseContextualError {
  constructor() {
    super('Something went wrong. Please reach out to the developers', 'INTERNAL_SERVER_ERROR', 500);
  }
};
